...
private:
    ...
    void createInstance() {
        ...
        applicationInfo.pApplicationName = "Hello Compute Shader";
    ...
    void populateDebugMessengerCreateInfo(VkDebugUtilsMessengerCreateInfoEXT& createInfo) {
        ...
        createInfo.pfnUserCallback = ComputeShaderApplication::debugCallback;
    ...
    void createDescriptorSetLayout() {
        ...
        computeLayoutBindings[0].stageFlags = VK_SHADER_STAGE_COMPUTE_BIT | VK_SHADER_STAGE_VERTEX_BIT;
    ...
    void createParticleSSBOBuffers() {
        ...
        for (auto& particle : particles) {
            ...
            particle.velocity = glm::normalize(glm::vec2(x, y)) * speed;
    ...
    void createDescriptorPool() {
        ...
        poolSizes2[0].descriptorCount = static_cast<uint32_t>(MAX_FRAMES_IN_FLIGHT);
        poolSizes2[1].type = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
        poolSizes2[1].descriptorCount = static_cast<uint32_t>(MAX_FRAMES_IN_FLIGHT) * 2;

        VkDescriptorPoolCreateInfo info{};
        info.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
        info.poolSizeCount = static_cast<uint32_t>(poolSizes2.size());
        info.pPoolSizes = poolSizes2.data();
        info.maxSets = static_cast<uint32_t>(MAX_FRAMES_IN_FLIGHT);

        if (vkCreateDescriptorPool(device, &info, nullptr, &computeDescriptorPool) != VK_SUCCESS) {
            throw std::runtime_error("Failed to create descriptor pool!");
        }
    }
    ...
    void createCommandBuffers() {
        ...
        allocInfo2.commandBufferCount = static_cast<uint32_t>(computeCommandBuffers.size());

        if (vkAllocateCommandBuffers(device, &allocInfo2, computeCommandBuffers.data()) != VK_SUCCESS) {
    ...
    void createGraphicsPipeline() {
        auto vertShaderCode = ComputeShaderApplication::readFile("shaders/vert.spv");
        auto fragShaderCode = ComputeShaderApplication::readFile("shaders/frag.spv");
        auto computeShaderCode = ComputeShaderApplication::readFile("shaders/particles.spv");
    ...
    void drawFrame() {
        ...
        recordComputeCommandBuffer(commandBuffers[currentFrame], computeCommandBuffers[currentFrame]);
    ...
    void recordComputeCommandBuffer(VkCommandBuffer commandBuffer, VkCommandBuffer computeCommandBuffer) {
        VkCommandBufferBeginInfo beginInfo{};
        beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
...
protected:
    ...
    std::vector<VkCommandBuffer> computeCommandBuffers; // cleaned with pool automatically
